...
## Objetivo
Construir um conjunto de dados especializado e curado para o treinamento de um modelo acoplado via LoRA (Low-Rank Adaptation), capaz de aprender, preservar e reproduzir com fidelidade as características estéticas, cromáticas e texturais da azulejaria luso-brasileira tradicional, moderna e contemporânea.
...
- **Formato Visual:** Imagens em formato `.png` de alta qualidade, livres de artefatos digitais.

## Origem e Governança de Dados
- **Fontes:** Wikimedia Commons, Unsplash e acervos catalogados.
...
- **Padrões Geométricos:** Composições modulares clássicas (estilo Pombalino) e ilusões de ótica.
- **Heráldica:** Brasões e escudos de armas detalhados.
- **Composições em Patchwork:** Mosaicos e colagens de azulejos variados ("colcha de retalhos").
...
- **Formatos Disponibilizados:**
  ...
  - **Arquivos Individuais:** 40 arquivos `.txt` sincronizados com as captions ajustadas (curadoria) dentro da pasta `dados/captions/` (padrão de compatibilidade para frameworks como Kohya_ss).